...
// Utility for clean numeric formatting (e.g., 1200 -> 1.2k)
...
export default function PropertiesCityChart({ data = [] }) {
    ...
    <div className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow duration-200 w-full min-w-0 flex flex-col gap-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 bg-slate-100 rounded-lg text-xs font-medium text-slate-600">
          <span>Total:</span>
          <span className="text-slate-900 font-semibold">{totalProperties.toLocaleString()}</span>
        </div>
      </div>

      {/* Chart Canvas */}
      <div className="w-full h-[260px] lg:max-w-[450px]">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            allowDecimals={false}
            /* Set explicit range: from 0 to 'dataMax + buffer' */
            domain={[0, (dataMax) => Math.ceil(dataMax * 1.2)]} 
            />

            <Tooltip
              content={<CustomTooltip />}
              cursor={false} // Disabled built-in cursor in favor of explicit cell hovering
            />
